...
def add_connection_to_airflow(connection_args):
    """Add new custom connection to Airflow database
    ...
    if connection_args['conn_id'] not in connections:
        try:
            ...
            # Gets data to be inserted
            data =  (connection_args['conn_id']
                    ,connection_args['conn_type']
                    ,connection_args['host']
                    ,connection_args['schema']
                    ,connection_args['login']
                    ,connection_args['password']
                    ,connection_args['port']
                    )

            # Insert data into Postgres database
            query = """Insert Into connection (conn_id, conn_type, host, schema, login, password, port)
...
def get_connection_args():
    """Defines and returns Connection Information to be added to Airflow database
...
def create_resources():
    ...
    # Getting Connections to be added to Airflow
    print('Adding Custom Connections to Airflow...')
    connection_args = get_connection_args()

    # Adding Connections to Airflow
    print(f"Adding {connection_args['aws']['conn_id']} Connection.")
    add_connection_to_airflow(connection_args['aws'])

    print(f"Adding {connection_args['redshift']['conn_id']} Connection.")
    add_connection_to_airflow(connection_args['redshift'])
